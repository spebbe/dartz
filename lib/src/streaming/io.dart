...
class IO {
  ...
  static final Conveyor<Free<IOOp, dynamic>, String> stdinReader = Source.eval/*<Free<IOOp, dynamic>, String>*/(readln).repeat().takeWhile((s) => s != null);

  static final Conveyor<Free<IOOp, dynamic>, SinkF<Free<IOOp, dynamic>, String>> stdoutWriter = Source.constant(IOM, (String s) => Source.eval_(println(s)));

  static Conveyor<Free<IOOp, dynamic>, IList<int>> fileReader(String path, [int chunkBytes = 4096]) => Source.resource(
      openFile(path, true),
      (FileRef file) => Source.eval /*<Free<IOOp, dynamic>, IList<int>>*/(readBytes(file, chunkBytes)).repeat().takeWhile((bytes) => bytes != nil()),
      (FileRef file) => Source.eval_(closeFile(file)));
  ...
  static Conveyor<Free<IOOp, dynamic>, SinkF<Free<IOOp, dynamic>, IList<int>>> fileWriter(String path) => Source.resource(
      openFile(path, false),
      (FileRef file) => Source.constant(IOM, (IList<int> bytes) => Source.eval_(writeBytes(file, bytes))),
      (FileRef file) => Source.eval_(closeFile(file)));
