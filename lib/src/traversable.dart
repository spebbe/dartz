...
abstract class TraversableOps<F, A> extends FunctorOps<F, A>
    with FoldableOps<F, A> {
  ...
  @override
  F map<B>(B f(A a)); // => cast(traverse<Object>(IdM, f));
...
class TraversableOpsTraversable<F extends TraversableOps>
    extends Traversable<F> {
  const TraversableOpsTraversable() : super._();

  //@override G traverse<G>(Applicative<G> gApplicative, F fa, G f(a));// => fa.traverse(gApplicative, f);
  @override
  B foldRight<A, B>(F fa, B z, B f(A a, B previous)) =>
      fa.foldRight(z, cast(f));

  @override
  B foldMap<A, B>(Monoid<B> bMonoid, F fa, B f(A a)) =>
      fa.foldMap(bMonoid, cast(f));

  @override
  F map<A, B>(F fa, B f(A a)) => cast(fa.map<B>(cast(f)));
  //@override G sequence_<G>(Applicative<GOptionTraversable> gApplicative, F fa) => fa.sequence_(gApplicative);
  ...
  @override
  Option<A> concatenateO<A>(Semigroup<A> si, F fa) => cast(fa.concatenateO(si));

  @override
  A concatenate<A>(Monoid<A> mi, F fa) => cast(fa.concatenate(mi));

  @override
  Option<B> foldMapO<A, B>(Semigroup<B> si, F fa, B f(A a)) =>
      fa.foldMapO(si, cast(f));

  @override
  B foldLeft<A, B>(F fa, B z, B f(B previous, A a)) => fa.foldLeft(z, cast(f));
}
